...
specification: |
  ...
  sentinel or a type: `E_LOCK` for flock, `E_LAYAT_BUILD` for a failure of an internal nix
  eval / build invocation, `E_NOTFOUND`, `E_PERMISSION`, `E_IO` for other filesystem /
  external I/O failures, and `E_LAYAT_COLLISION` for the conflict of `apply --dryrun`
  (exit 2). Anything else SHALL fall back to the tool-generic `E_LAYAT_FAILED`.
specification_ja: |
  ...
  `E_LAYAT_BUILD`（内部 nix eval / build 呼び出しの失敗）/ `E_NOTFOUND` / `E_PERMISSION` /
  `E_IO`（それ以外の FS / 外部 I/O 失敗）/ `E_LAYAT_COLLISION`（`apply --dryrun` の
  conflict = exit 2）。それ以外はツール総称 fallback `E_LAYAT_FAILED` としなければならない。
---
...
`E_LAYAT_BUILD`（内部 nix eval / build 呼び出しの失敗）/ `E_NOTFOUND` / `E_PERMISSION` /
`E_IO`（上記に該当しない FS / 外部 I/O 失敗の形〔PathError 等〕）/ `E_LAYAT_COLLISION`
（`apply --dryrun` の conflict = exit 2）。それ以外はツール総称 fallback
`E_LAYAT_FAILED`（世代 commit 失敗等）。
