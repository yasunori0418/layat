...
### nput CLI
ユーザーが直接触れる一次 UX。`PATH` 上の `packages.nput` バイナリ。**entrypoint** を発見し、内部で `nix build` / `eval` を回して named manifest を取得し、エンジンに配置させる。サブコマンドは `apply [<name>]`・`apply --all`・`reset`・`rollback`・`list-generations`・`gitignore`・`prune`・`init`。
