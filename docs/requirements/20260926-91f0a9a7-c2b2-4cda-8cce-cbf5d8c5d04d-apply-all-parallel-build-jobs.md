...
specification: |
  ...
  resolved value of `--jobs`; stage 2 SHALL then apply the configs one by one in lexical
  order, with the build still performed inside the engine's lock. `--jobs` SHALL be a flag
...
specification_ja: |
  ...
  ならない。stage 2 はその後 config を辞書順に 1 つずつ適用しなければならず、build は従来どおり
  engine のロック内で行わなければならない。`--jobs` は `apply` ローカルのフラグでなければ
...
- **2 段構成**: stage 1 = 選択 config の build を `--no-link` で先行 realize（worker pool・
  同時実行数 ≤ `--jobs`）、stage 2 = 辞書順逐次の適用。engine の in-lock build は残り、
...
- **決定性**: 集約結果は `--jobs` に依存しない。非決定なのは `--debug` 開示行の順序だけで、
  stage 1 の nix 自身の出力は捕捉されて失敗時の error に載る

> **上は要約で、規範は frontmatter が正**。配置段階の並列化は後続（Issue #154）の担当。
