...
cachix のキャッシュ `yasunori0418` へ `packages.<system>.layat` のビルド生成パスを投入する。
consumer の CI / ローカルが「最新 main の `layat` を実ビルドせずキャッシュから引く」ための基盤。
...
- 構成は CI パイプライン（INF-d1230e1f-8ba8-49d8-8386-409bfbb7dd27）と同じ os×system の 3 環境マトリクス
  ...
  `macos-latest`=aarch64-darwin）で `nix build .#packages.<system>.layat` をネイティブ実行する
...
`push: main` + `paths` + `workflow_dispatch`。layat バイナリの内容を決める入力
...
されず、layat が flake input として消費される形と噛み合わないため。ここは INF-9878e9f5-1ec0-4ef2-879b-859ea538cc43
