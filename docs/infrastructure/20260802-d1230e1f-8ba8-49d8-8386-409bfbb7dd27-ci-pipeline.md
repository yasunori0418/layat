...
| `e2e` | CI 用 devShell から E2E ハーネスを走らせ、フィクスチャへ `layat apply` して FS / nix profile / rollback をアサートする | `ubuntu-latest` |
...
Go の check（go-vet / golangci-lint）は `CGO_ENABLED=0` のピュア Go で実行する。layat は
...
`e2e` は `nix develop '.?dir=dev#ci' -c tests/e2e/run.sh`。ハーネスは `layat` / `git` / `jq` /
