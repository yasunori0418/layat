...
不在でもエラーにしない。基底が存在するのに `ReadDir` が権限等で失敗するときは、その基底を
名指しした warning が出て、もう一方の基底の処理は続く。**「基底が無い」と「基底が読めない」が
区別される**ことの担保（→ RISK-f522a51b-db4b-4164-bd20-63fa4a0cb27d）。
