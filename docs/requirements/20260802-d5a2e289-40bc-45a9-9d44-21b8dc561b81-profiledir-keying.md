...
- **`--root` 明示時は全モードで profileDir を上書き後 root の `<roothash>` でキーする**。
  home / fixed mode も `<state>/nix/profiles/layat/<roothash>/<name>` になり、異なる
...
> `<state>/nix/profiles/layat/default`（固定名 1 profile）」の行は、**ADR-0035 が HM の
> `layat.configs.<name>` 実装を決定済み**で反転している（HM も `<name>` 次元を持ち、home mode の
