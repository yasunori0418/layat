...
specification: |
  ...
  (`packages = [ layat.packages.${system}.layat ]`, `shellHook` being a named apply), and a
  comment stating that placed objects are ephemeral and that the output of
  `layat gitignore <name>` should be appended to `.gitignore`; it SHALL also provide a
  `.gitignore` skeleton headed by the comment
  `# layat: regenerate with 'layat gitignore <name>'`. The `shellHook` of the `project`
  template SHALL default to the named `layat apply <name> --no-wait`, that being clearest
  for a one-config example and free of the mixed-mode footgun, and SHALL present
  `layat apply --all --project-root --no-wait` for multiple configs as a comment. An
...
specification_ja: |
  ...
  + devShell（`packages = [ layat.packages.${system}.layat ]`・`shellHook` は名指し apply）+
  「配置物は ephemeral・`.gitignore` へ `layat gitignore <name>` 出力を追記」コメントを持つ
  `flake.nix` と、`# layat: regenerate with 'layat gitignore <name>'` ヘッダコメント付きの
  `.gitignore` 雛形を提供しなければならない。`project` template の `shellHook` は名指しの
  `layat apply <name> --no-wait` を既定としなければならない（example が 1 config なので
  最も明確で混在 footgun が起きないため）。複数 config 向けの
  `layat apply --all --project-root --no-wait` はコメントで示さなければならない。
  `.envrc`（direnv）は
...
| template | ファイル | 内容 |
|---|---|---|
| `standalone` | `flake.nix` | `homeRoot` の 1 config 例（`layat.<system>.<name> = mkManifest { root = homeRoot; entries = {...}; }`）+ バリエーションコメント |
| `project` | `flake.nix` | `projectRoot` の 1 config 例 + devShell（`packages = [ layat.packages.${system}.layat ]`・shellHook = 名指し apply）+ 「配置物は ephemeral・`.gitignore` へ `layat gitignore <name>` 出力を追記」コメント |
| `project` | `.gitignore` | 先頭に `# layat: regenerate with 'layat gitignore <name>'` ヘッダコメント付きの雛形 |

- project template の `shellHook` は **`layat apply <name> --no-wait`（名指し）**を既定に
  する。example が 1 config なので最も明確で混在 footgun が起きない。「複数 config なら
  `layat apply --all --project-root --no-wait`」をコメントで示す。
...
`docs/spec.md`「CLI 仕様」→「`layat init`」→「テンプレートの内容（最小 + 手厚いコメント）」。
