...
**削除権限が無い系列** — 削除対象と判定した系列に書き込み権限が無いとき、その系列は残って
`Skipped` に `permission-denied` として現れ、**同じ実行の他の系列の削除は続く**。1 系列の権限
不足が全体を止めないことの担保（→ ADR-0036 §3 の非 root 実行）。
...
**削除中の失敗** — 権限以外の理由で削除の途中で失敗した系列は `Removed` に数えず、その系列を
特定できる形でエラーが返る。判定段の `Skipped` とは別扱いであること、失敗より前に完了した系列は
`Removed` に残ることを見る。加えて、**失敗した系列に `.root` が残っており、再実行で同じ系列が
ふたたび走査対象になる**ことを確かめる（`.root` が先に消える実装は「`Removed` に入らずエラーが
返る」だけなら素通りするが、系列は二度と辿れなくなる → RISK-0909e121-8ec4-40c8-ab1c-562e61f31067）。
