...
出るだけになる。`lib` 直接利用者のうち、CLI かモジュールのいずれかを通す運用（大半がこれにあたる）は
上の 2 層で到達する。`mkManifest` の結果を自前スクリプトで配置する運用には 2 層とも届かないが、
その到達手段のために層を 1 つ維持するだけの数は見込めない。README のバナーと移行節・リポジトリ
説明文・epic のピン留めが到達性を補う。
